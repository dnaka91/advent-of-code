...
where
    ...
    P2: Fn(&'a str) -> T2 + 'a,
{
    c.bench_function(&format!("Y2017 D{:02} P1", day), |b| b.iter(|| part1(black_box(input))));
    c.bench_function(&format!("Y2017 D{:02} P2", day), |b| b.iter(|| part2(black_box(input))));
}

criterion_group!(benches, y2017);
