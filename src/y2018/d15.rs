...
//! to **move** into range of an enemy (if it isn't already) and then **attack** (if it is in
//! range).
//!
...
//! and does not do any prediction about where units will be later. If the unit cannot reach (find
//! an open path to) any of the squares that are in range, it ends its turn. If multiple squares are
//! in range and **tied** for being reachable in the fewest steps, the square which is first in
...
//! and so combat ends. So, the number of **full rounds** that were completed is **`47`**, and the
//! sum of the hit points of all remaining units is `200+131+59+200 = 590`. From these, the
