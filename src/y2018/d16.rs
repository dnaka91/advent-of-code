...
//! instruction and how the inputs are interpreted. The output, `C`, is always treated as a
//! register.
//!
...
//!   `B`.
//! - `addi` (add immediate) stores into register `C` the result of adding register `A` and value
//!   `B`.
//!
//! Multiplication:
//!
...
//!   has a value of `2`) produces `2`, which matches the value stored in the output register,
//!   register `2`.
