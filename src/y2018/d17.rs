...
//! ```
//!
//! How many tiles can be reached by the water? **To prevent counting forever**, ignore tiles with a
//! `y` coordinate smaller than the smallest `y` coordinate in your scan data or larger than the
//! largest one. Any `x` coordinate is valid. In this example, the lowest `y` coordinate given is
//! `1`, and the highest is `13`, causing the water spring (in row `0`) and the water falling off
