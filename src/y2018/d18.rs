...
//! (`.`), **trees** (`|`), or a **lumberyard** (`#`). You take a scan of the area (your puzzle
//! input).
//!
//! Strange magic is at work here: each minute, the landscape looks entirely different. In exactly
//! **one minute**, an open acre can fill with trees, a wooded acre can be converted to a
//! lumberyard, or a lumberyard can be cleared to open ground (the lumber having been sent to other
//! projects).
//!
//! The change to each acre is based entirely on **the contents of that acre** as well as **the
//! number of open, wooded, or lumberyard acres adjacent to it** at the start of each minute. Here,
//! "adjacent" means any of the eight acres surrounding that acre. (Acres on the edges of the lumber
//! collection area might have fewer than eight adjacent acres; the missing acres aren't counted.)
//!
//! In particular:
//!
...
//!   least one other lumberyard and at least one acre containing trees**. Otherwise, it becomes
//!   **open**.
//!
//! These changes happen across all acres **simultaneously**, each of them using the state of all
//! acres at the beginning of the minute and changing to their new form by the end of that same
