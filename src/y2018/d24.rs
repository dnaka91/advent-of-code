...
//!
//! After [a weird buzzing noise], you appear back at the man's cottage. He seems relieved to see
//! his friend, but quickly notices that the little reindeer caught some kind of cold while out
//! exploring.
//!
//! The portly man explains that this reindeer's immune system isn't similar to regular reindeer
//! immune systems:
//!
...
//! their attack damage. The above group has an effective power of `18 * 8 = 144`. Groups never have
//! zero or negative units; instead, the group is removed from combat.
//!
//! Each **fight** consists of two phases: **target selection** and **attacking**.
//!
//! During the **target selection** phase, each group attempts to choose one target. In decreasing
...
//! deal damage equal to its **effective power** to the defending group. However, if the defending
//! group is **immune** to the attacking group's attack type, the defending group instead takes **no
//! damage**; if the defending group is **weak** to the attacking group's attack type, the defending
...
//! immediately kill it is ignored. For example, if a defending group contains `10` units with `10`
//! hit points each and receives `75` damage, it loses exactly `7` units and is left with `3` units
//! at full health.
//!
//! After the fight is over, if both armies still contain units, a new fight begins; combat only
//! ends once one army has lost all of its units.
//!
...
//!
//! You scan the reindeer's condition (your puzzle input); the white-bearded man looks nervous. As
//! it stands now, **how many units would the winning army have**?
//!
