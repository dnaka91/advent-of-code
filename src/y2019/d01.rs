...
//!   `3` and rounded down is `0`, which would call for a negative fuel), so the total fuel required
//!   is still just `2`.
...
pub fn solve_part_one(input: &str) -> Result<i64> {
    Ok(input.lines().map(|l| l.parse::<i64>().unwrap()).map(fuel_for_mass).sum())
}

pub fn solve_part_two(input: &str) -> Result<i64> {
    let mut total = 0;

    for module in input.lines().map(|l| l.parse::<i64>().unwrap()) {
        let mut fuel = fuel_for_mass(module);
        while fuel > 0 {
            total += fuel;
...
mod tests {
    ...
    #[test]
    fn part_one() {
        assert_eq!(2, solve_part_one("12").unwrap());
        assert_eq!(2, solve_part_one("14").unwrap());
        assert_eq!(654, solve_part_one("1969").unwrap());
        assert_eq!(33583, solve_part_one("100756").unwrap());
    }

    #[test]
    fn part_two() {
        assert_eq!(2, solve_part_two("12").unwrap());
        assert_eq!(966, solve_part_two("1969").unwrap());
        assert_eq!(50346, solve_part_two("100756").unwrap());
    }
}
