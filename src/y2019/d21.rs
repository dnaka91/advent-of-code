...
//! available: `T`, the **temporary value** register, and `J`, the **jump register**. If the jump
//! register is **true** at the end of the springscript program, the springdroid will try to jump.
...
//! the direction it is facing; these values are provided in **read-only registers**. Your
//! springdroid can detect ground at four distances: one tile away (`A`), two tiles away (`B`),
//! three tiles away (`C`), and four tiles away (`D`). If there is ground at the given distance, the
...
//!
//! In all three instructions, the second argument (`Y`) needs to be a **writable register** (either
//! `T` or `J`). The first argument (`X`) can be **any register** (including `A`, `B`, `C`, or `D`).
//!
...
//! `10`). **When you have finished entering your program**, provide the command `WALK` followed by
//! a newline to instruct the springdroid to begin surveying the hull.
//!
//! If the springdroid **falls into space**, an ASCII rendering of the last moments of its life will
//! be produced. In these, `@` is the springdroid, `#` is hull, and `.` is empty space. For example,
//! suppose you program the springdroid like this:
//!
//! ```txt
//! NOT D J
//! WALK
//! ```
//!
//! This one-instruction program sets `J` to **true** if and only if there is no ground four tiles
//! away. In other words, it attempts to jump into any hole it finds:
//!
//! ```txt
//! .................
//! .................
