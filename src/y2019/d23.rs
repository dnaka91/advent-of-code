...
//! over the network by sending and receiving **packets**. All packets contain **two values** named
//! `X` and `Y`. Packets sent to a computer are queued by the recipient and read in the order they
//! are received.
//!
//! To **send** a packet to another computer, the NIC will use **three output instructions** that
//! provide the **destination address** of the packet followed by its `X` and `Y` values. For
//! example, three output instructions that provide the values `10`, `20`, `30` would send a packet
...
//! queue.
//!
//! Note that these input and output instructions never [block]. Specifically, output instructions
//! do not wait for the sent packet to be received - the computer might send multiple packets before
//! receiving any. Similarly, input instructions do not wait for a packet to arrive - if no packet
//! is waiting, input instructions should receive `-1`.
//!
