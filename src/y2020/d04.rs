...
pub fn solve_part_one(input: &str) -> Result<usize> {
    Ok(parse_input(input)?
        .into_iter()
        .filter(|p| {
            ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"].iter().all(|f| p.contains_key(f))
        })
        .count())
}
    ...
    Ok(parse_input(input)?
        ...
        .filter(|p| {
            ...
            let hgt = p
                .get("hgt")
                .filter(|v| v.len() > 2)
                .and_then(|v| v[..v.len() - 2].parse::<u8>().ok().map(|h| (h, &v[v.len() - 2..])))
                .filter(|(h, s)| match *s {
                    "cm" => 150 <= *h && *h <= 193,
                    "in" => 59 <= *h && *h <= 76,
                    _ => false,
            ...
            let hcl = p
                ...
                .filter(|v| {
                    v.len() == 7
                        && v.chars().next().map(|c| c == '#').unwrap_or_default()
                        && v.chars().skip(1).all(|c| matches!(c, '0'..='9' | 'a'..='f'))
                })
                .is_some();
            let ecl = p
                .get("ecl")
            ...
            let pid = p
                .get("pid")
                .filter(|v| v.len() == 9 && v.chars().all(|c| c.is_ascii_digit()))
                .is_some();
        ...
        .map(|pp| {
            pp.split_terminator(|c| c == ' ' || c == '\n')
                .map(|e| {
                    ...
                    let v = parts.next().context("value missing")?;
                    Ok((k, v))
                })
...
mod tests {
    ...
    fn part_two() {
        ...
        assert_eq!(0, solve_part_two(input).unwrap());
