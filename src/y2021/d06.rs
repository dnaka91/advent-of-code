...
//! included as a valid timer value). The new lanternfish starts with an internal timer of `8` and
...
pub fn solve_part_one(input: &str) -> Result<u64> {
    let fiiishy = parse_input(input)?;
    Ok(grow_fiiish(fiiishy, 80))
}

pub fn solve_part_two(input: &str) -> Result<u64> {
    let fiiishy = parse_input(input)?;
    Ok(grow_fiiish(fiiishy, 256))
}
...
fn parse_input(input: &str) -> Result<Vec<u8>> {
    input
        ...
        .map(|fish| {
            let fish = fish.parse()?;
            ensure!(fish <= 6, "fishes must not be older than 6 days");
            Ok(fish)
        })
        .collect()
}

fn grow_fiiish(fiiishy: Vec<u8>, days: usize) -> u64 {
    let mut growth = [0u64; 9];

    for fiiish in fiiishy {
        growth[fiiish as usize] += 1;
    }

    for _ in 0..days {
        let moar_fiiish = growth[0];

        for i in 1..9 {
            growth[i - 1] = growth[i];
        }

        growth[6] += moar_fiiish;
        growth[8] = moar_fiiish;
    }

    growth.into_iter().sum()
}
...
mod tests {
    ...
    #[test]
    fn part_one() {
        assert_eq!(5934, solve_part_one(INPUT).unwrap());
    }

    #[test]
    fn part_two() {
        assert_eq!(26984457539, solve_part_two(INPUT).unwrap());
    }
}
