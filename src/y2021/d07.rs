...
pub fn solve_part_one(input: &str) -> Result<i32> {
    ...
    (0..crabs.len() as i32)
        .map(|pos| crabs.iter().copied().map(|crab| (crab - pos).abs()).sum())
        .min()
        .context("no crabs in the input")
}
...
fn parse_input(input: &str) -> Result<Vec<i32>> {
    input
        ...
        .split(',')
        .map(|crab| crab.parse().map_err(Into::into))
        .collect()
}
...
mod tests {
    ...
    #[test]
    fn part_two() {
        assert_eq!(168, solve_part_two(INPUT).unwrap());
    }
}
