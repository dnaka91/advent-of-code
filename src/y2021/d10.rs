...
//! Then, for each character, multiply the total score by 5 and then increase the total score by the
...
//! then taking the **middle** score. (There will always be an odd number of scores to consider.) In
//! this example, the middle score is **`288957`** because there are the same number of scores
...
pub fn solve_part_one(input: &str) -> Result<u64> {
    let systems = parse_input(input)?;
    let mut stack = Vec::new();

    Ok(systems
        .into_iter()
        .filter_map(|system| {
            for (ch, open) in system {
                if open {
                    stack.push(ch);
                } else if let Some(points) =
                    stack.pop().and_then(|ch2| (ch2 != ch).then(|| ch.points_one()))
                {
                    stack.clear();
                    return Some(points);
                }
...
pub fn solve_part_two(input: &str) -> Result<u64> {
    ...
    let mut scores = systems
        .into_iter()
        .filter_map(|system| {
            for (ch, open) in system {
                if open {
                    stack.push(ch);
                } else if stack.pop().map(|ch2| ch2 != ch).unwrap_or(true) {
                    stack.clear();
                    return None;
                }
            }

            Some(stack.drain(0..stack.len()).rev().fold(0, |acc, ch| acc * 5 + ch.points_two()))
        })
        .collect::<Vec<_>>();

    scores.sort_unstable();
    scores.get(scores.len() / 2).copied().context("no scores")
}

fn parse_input(input: &str) -> Result<Vec<Vec<(Character, bool)>>> {
    input
        .lines()
        .map(|system| {
            system
                ...
                .map(|ch| {
                    Ok(match ch {
                        b'(' => (Character::Round, true),
                        b')' => (Character::Round, false),
                        b'[' => (Character::Square, true),
                        b']' => (Character::Square, false),
                        b'{' => (Character::Curly, true),
                        b'}' => (Character::Curly, false),
                        b'<' => (Character::Pointy, true),
                        b'>' => (Character::Pointy, false),
                        _ => bail!("invalid character `{}`", ch as char),
                    })
                })
                .collect()
        })
...
mod tests {
    ...
    #[test]
    fn part_one() {
        assert_eq!(26397, solve_part_one(INPUT).unwrap());
    }

    #[test]
    fn part_two() {
        assert_eq!(288957, solve_part_two(INPUT).unwrap());
    }
}
