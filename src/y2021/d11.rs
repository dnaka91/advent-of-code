...
//!   process continues as long as new octopuses keep having their energy level increased beyond
...
pub fn solve_part_one(input: &str) -> Result<u32> {
    let mut levels = parse_input(input)?;
    let mut flashed = AHashSet::with_capacity(levels.len());
...
fn parse_input(input: &str) -> Result<[u8; 100]> {
    let mut lines = input.lines();
    let mut values = [0; 100];

    for i in 0..10 {
        let line = lines.next().context("grid has less than 10 rows")?;
...
fn bubble(levels: &mut [u8; 100], pos: usize, flashed: &mut AHashSet<usize>) -> u32 {
    ...
    if levels[pos] > 9 {
        ...
        if pos >= 10 {
            count += bubble(levels, pos - 10, flashed);

            if pos % 10 != 0 {
                count += bubble(levels, pos - 11, flashed);
            }

            if pos % 10 != 9 {
                count += bubble(levels, pos - 9, flashed);
            }
        }
        ...
        if pos % 10 != 0 {
            count += bubble(levels, pos - 1, flashed);
        }
...
mod tests {
    ...
    #[test]
    fn part_one() {
        assert_eq!(1656, solve_part_one(INPUT).unwrap());
    }

    #[test]
    fn part_two() {
        assert_eq!(195, solve_part_two(INPUT).unwrap());
    }
}
