...
//! step, the probe's `x` position is at least `20` and at most `30`, **and** the probe's `y`
...
//! right, and the `y` coordinate increases upward. In the bottom right, positions that are within
//! the target area are shown as `T`. After each step (until the target area is reached), the
//! position of the probe is marked with `#`. (The bottom-right `#` is both a position the probe
