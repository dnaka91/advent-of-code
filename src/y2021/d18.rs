...
//! list of actions. For example, if **split** produces a pair that meets the **explode** criteria,
...
//!
//! - `[[[[[9,8],1],2],3],4]` becomes `[[[[0,9],2],3],4]` (the `9` has no regular number to its
//!   left, so it is not added to any regular number).
//! - `[7,[6,[5,[4,[3,2]]]]]` becomes `[7,[6,[5,[7,0]]]]` (the `2` has no regular number to its
//!   right, and so it is not added to any regular number).
//! - `[[6,[5,[4,[3,2]]]],1]` becomes `[[6,[5,[7,0]]],3]`.
//! - `[[3,[2,[1,[7,3]]]],[6,[5,[4,[3,2]]]]]` becomes `[[3,[2,[8,0]]],[9,[5,[4,[3,2]]]]]` (the pair
//!   `[3,2]` is unaffected because the pair `[7,3]` is further to the left; `[3,2]` would explode
//!   on the next action).
...
//! ```
//!
//! The final sum `[[[[8,7],[7,7]],[[8,6],[7,7]]],[[[0,7],[6,6]],[8,7]]]` is found after adding up
//! the above snailfish numbers:
//!
//! ```txt
//!   [[[0,[4,5]],[0,0]],[[[4,5],[2,6]],[9,5]]]
//! + [7,[[[3,7],[4,3]],[[6,3],[8,8]]]]
...
//! `3*1 + 2*9 = 21`. Magnitude calculations are recursive: the magnitude of `[[9,1],[1,9]]` is
//! `3*29 + 2*21 = 129`.
//!
