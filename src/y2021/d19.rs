...
//! **scanners** into the water. It's difficult to navigate in the pitch black open waters of the
//! ocean trench, but if you can build a map of the trench using data from the scanners, you should
...
//! cube centered on the scanner; beacons that are at most 1000 units away from the scanner in each
//! of the three axes (`x`, `y`, and `z`) have their precise position determined relative to the
//! scanner. However, scanners cannot detect other scanners. The submarine has automatically
...
//! might agree on which direction is positive `x`, but one scanner might be upside-down from the
//! perspective of the other scanner. In total, each scanner could be in any of 24 different
...
//! expressed relative to scanner `0` (using the orientation of scanner `0` and as if scanner `0` is
//! at coordinates `0,0,0`).
//!
//! Scanners `0` and `1` have overlapping detection cubes; the 12 beacons they both detect (relative
//! to scanner `0`) are at the following coordinates:
//!
//! ```txt
//! -618,-824,-621
//! -537,-823,-458
...
//!
//! Scanner `4` overlaps with scanner `1`; the 12 beacons they both detect (relative to scanner `0`)
//! are:
//!
//! ```txt
//! 459,-707,401
...
//!
//! Following this process, scanner `2` must be at `1105,-1205,1229` (relative to scanner `0`) and
