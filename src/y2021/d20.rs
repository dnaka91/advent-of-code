...
//! and (6,11). These nine input pixels are combined into a single binary number that is used as an
...
//! `.#.`; combining these forms `...#...#.`. By turning dark pixels (`.`) into `0` and light pixels
...
//! size. **Every** pixel of the infinite output image needs to be calculated exactly based on the
//! relevant pixels of the input image. The small input image you have is only a small region of the
//! actual infinite input image; the rest of the input image consists of dark pixels (`.`). For the
//! purposes of the example, to save on space, only a portion of the infinite-sized input and output
