...
//! the start of the reboot process, they are all **off**. (Could it be an old model of a reactor
//! you've seen [before]?)
//!
//! To reboot the reactor, you just need to set all of the cubes to either **on** or **off** by
//! following a list of **reboot steps** (your puzzle input). Each step specifies a [cuboid] (the
//! set of all cubes that have coordinates which fall within ranges for `x`, `y`, and `z`) and
//! whether to turn all of the cubes in that cuboid **on** or **off**.
//!
...
//! - `12,12,12`
//!
//! The second step (`on x=11..13,y=11..13,z=11..13`) turns **on** a 3x3x3 cuboid that overlaps with
//! the first. As a result, only 19 additional cubes turn on; the rest are already on from the
//! previous step:
//!
//! - `11,11,13`
//! - `11,12,13`
...
//! - `11,11,11`
//!
//! The final step (`on x=10..10,y=10..10,z=10..10`) turns **on** a single cube, `10,10,10`. After
//! this last step, **`39`** cubes are **on**.
//!
