...
//!   their own destination. If an amphipod's starting room is not its destination room, it can stay
//!   in that room until it leaves the room. (For example, an Amber amphipod will not move from the
//!   hallway into the right three rooms, and will only move into the leftmost room if that room is
...
//!   into a room**. (That is, once any amphipod starts moving, any other amphipods currently in the
//!   hallway are locked in place and will not move again until they can move fully into a room.)
//!
//! In the above example, the amphipods can be organized using a minimum of **`12521`** energy. One
