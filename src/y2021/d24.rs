...
//!
//! [Magic smoke] starts leaking from the submarine's [arithmetic logic unit] (ALU). Without the
//! ability to perform basic arithmetic and logic functions, the submarine can't produce cool
...
//! - `mul a b` - Multiply the value of `a` by the value of `b`, then store the result in variable
//!   `a`.
//! - `div a b` - Divide the value of `a` by the value of `b`, truncate the result to an integer,
//!   then store the result in variable `a`. (Here, "truncate" means to round the value toward
//!   zero.)
//! - `mod a b` - Divide the value of `a` by the value of `b`, then store the remainder in variable
//!   `a`. (This is also called the [modulo] operation.)
...
//!
//! The ALU has no **jump** instructions; in an ALU program, every instruction is run exactly once
//! in order from top to bottom. The program halts after the last instruction has finished
//! executing.
//!
