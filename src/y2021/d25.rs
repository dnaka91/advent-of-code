...
//! submarine's experimental antenna **still isn't boosted enough** to detect the keys, but they
//! **must** be here. All you need to do is **reach the seafloor** and find them.
//!
...
//! while the other always moves **south** (`v`). Each location can contain at most one sea
//! cucumber; the remaining locations are **empty** (`.`). The submarine helpfully generates a map
...
//! then the sea cucumbers in the south-facing herd attempt to move forward one location. When a
//! herd moves forward, every sea cucumber in the herd first simultaneously considers whether there
//! is a sea cucumber in the adjacent location it's facing (even another sea cucumber facing the
//! same direction), and then every sea cucumber facing an empty location simultaneously moves into
//! that location.
//!
