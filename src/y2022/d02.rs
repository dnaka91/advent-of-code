...
pub fn solve_part_one(input: &str) -> Result<u32> {
    Ok(parse_input::<Shape>(input)?
        .into_iter()
        .map(|(opponent, me)| me.points(opponent) + me.value())
        .sum())
}

pub fn solve_part_two(input: &str) -> Result<u32> {
    Ok(parse_input::<Choice>(input)?
        .into_iter()
        .map(|(opponent, choice)| choice.points() + Shape::for_choice(choice, opponent).value())
        .sum())
}
...
impl Shape {
    ...
    fn for_choice(choice: Choice, opponent: Self) -> Self {
        match choice {
            Choice::Lose => match opponent {
                Self::Rock => Self::Scissors,
                Self::Paper => Self::Rock,
                Self::Scissors => Self::Paper,
            },
            Choice::Draw => opponent,
            Choice::Win => match opponent {
...
fn parse_input<T>(input: &str) -> Result<Vec<(Shape, T)>>
where
    ...
    input
        ...
        .map(|line| {
            let (opponent, me) = line
                ...
                .context("value pair must be separated by space")?;
            Ok((opponent.parse()?, me.parse()?))
        })
        .collect()
}
