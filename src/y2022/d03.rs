...
//! then all three Elves will have item type `B` somewhere in their rucksack, and at most two of the
...
//! to tell which item type is the right one is by finding the one item type that is **common
...
pub fn solve_part_one(input: &str) -> Result<u32> {
    ...
    input
        ...
        .map(|line| {
            ...
            bag.clear();
            bag.extend(left.bytes());

            right.bytes().find(|c| bag.contains(c)).map(priority).unwrap_or(Ok(0))
        })
        .sum()
}

pub fn solve_part_two(input: &str) -> Result<u32> {
    let mut bag1 = AHashSet::new();
    let mut bag2 = AHashSet::new();
    ...
    input
        ...
        .map(|mut chunk| {
            bag1.clear();
            bag1.extend(chunk.next().context("missing first bag")?.bytes());
            bag2.clear();
            bag2.extend(chunk.next().context("missing second bag")?.bytes());

            chunk
                .next()
                .context("missing third bag")?
                .bytes()
                .find(|c| bag1.contains(c) && bag2.contains(c))
                .map(priority)
                .unwrap_or(Ok(0))
        })
        .sum()
...
fn priority(value: u8) -> Result<u32> {
    Ok(match value {
        b'a'..=b'z' => value - b'a' + 1,
        b'A'..=b'Z' => value - b'A' + 27,
        _ => bail!("invalid item type `{}`", value as char),
    } as u32)
}
