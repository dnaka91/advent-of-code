...
//! **fewest number of cubes of each color** that could have been in the bag to make the game
//! possible?
//!
...
pub fn solve_part_two(input: &str) -> Result<u32> {
    Ok(parse_input(input)?
        .into_iter()
        .map(|game| {
            let fewest = game.rounds.into_iter().fold(Round::default(), |acc, round| Round {
...
fn parse_input(input: &str) -> Result<Vec<Game>> {
    input
        ...
        .map(|line| {
            let (game, rounds) =
                line.split_once(": ").context("missing separator for game and rounds")?;
            let game_id = game
                .strip_prefix("Game ")
                .map(|id| id.parse::<u32>())
                    ...
                    round
                        ...
                        .map(|value| {
                            let (count, color) = value
                                .split_once(' ')
                                .context("missing separator for count and color")?;
                            let count = count.parse::<u32>().context("invalid count")?;

                            anyhow::Ok((count, color))
                        })
                        .try_fold(Round::default(), |mut round, result| {
...
mod tests {
    ...
    #[test]
    fn part_two() {
        assert_eq!(2286, solve_part_two(INPUT).unwrap());
    }
}
