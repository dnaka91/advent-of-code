...
//! have** appear in the list of **winning numbers**. The first match makes the card worth **one
...
//! have, four of them (`48`, `83`, `17`, and `86`) are winning numbers! That means card 1 is worth
...
pub fn solve_part_one(input: &str) -> Result<u32> {
    Ok(parse_input(input)?
        .into_iter()
        .map(|card| {
            let count = card.having.intersection(&card.winning).count() as u32;
            if count > 0 {
                2_u32.pow(count - 1)
            } else {
                0
...
pub fn solve_part_two(input: &str) -> Result<u32> {
    ...
    Ok(parse_input(input)?
        ...
        .map(|card| {
            let count = card.having.intersection(&card.winning).count() as u32;
            let copies = bag.remove(&card.id).unwrap_or(0) + 1;

            for id in card.id + 1..card.id + 1 + count {
                *bag.entry(id).or_default() += copies;
            }
...
fn parse_input(input: &str) -> Result<Vec<Card>> {
    input
        ...
        .map(|line| {
            let (card, content) =
                line.split_once(':').context("missing card and content separator")?;
            let id = card
                .strip_prefix("Card ")
                .context("missing `Card ` prefix")?
                .trim()
                .parse()
                .context("invalid card ID")?;

            let (winning, having) =
                content.split_once('|').context("missing winning and having number separator")?;
            let winning = winning
                .split_whitespace()
                .map(|v| v.parse().context("invalid number"))
                .collect::<Result<_>>()?;
            let having = having
                .split_whitespace()
                .map(|v| v.parse().context("invalid number"))
...
mod tests {
    ...
    #[test]
    fn part_one() {
        assert_eq!(13, solve_part_one(INPUT).unwrap());
    }

    #[test]
    fn part_two() {
        assert_eq!(30, solve_part_two(INPUT).unwrap());
    }
}
