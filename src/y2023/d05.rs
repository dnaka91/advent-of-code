...
//! snow with dirty water. Don't worry, I'm sure we'll get more sand soon; we only turned off the
//! water a few days... weeks... oh no." His face sinks into a look of horrified realization.
//!
...
//! ferry, maybe you can help us with our **food production problem**. The latest Island Island
...
//! maps describe entire **ranges** of numbers that can be converted. Each line within a map
...
//! a **range length** of `2`. This line means that the source range starts at `98` and contains two
...
//! through other categories until you can find its corresponding **location number**. In this
...
//! **start** of the range and the second value is the **length** of the range. So, in the first
...
//! corresponds to soil `84`, fertilizer `84`, water `84`, light `77`, temperature `45`, humidity
//! `46`, and **location `46`**. So, the lowest location number is **`46`**.
//!
...
pub fn solve_part_one(input: &str) -> Result<u64> {
    ...
    Ok(seeds.into_iter().map(|seed| transform_seed(seed, &maps)).min().unwrap_or(0))
}

pub fn solve_part_two(input: &str) -> Result<u64> {
    let (seeds, maps) = parse_input(input)?;
    ensure!(seeds.len() % 2 == 0, "seeds must contain an even number of elements");
    ...
    Ok(seeds
        ...
        .map(|seed| transform_seed(seed, &maps))
        .min()
        .unwrap_or(0))
}

fn transform_seed(seed: u64, maps: &[Vec<Convert>]) -> u64 {
    maps.iter().fold(seed, move |seed, map| {
        map.iter()
            .find(|convert| (convert.source..convert.source + convert.range).contains(&seed))
            .map(|convert| seed - convert.source + convert.destination)
            .unwrap_or(seed)
    })
}

fn parse_input(input: &str) -> Result<(Vec<u64>, Vec<Vec<Convert>>)> {
    let mut parts = input.split("\n\n");

    let seeds = parts.next().context("no seeds")?;
    let seeds = seeds
        .strip_prefix("seeds: ")
        .context("missing seeds prefix")?
        .split_ascii_whitespace()
        .map(|v| v.parse().context("invalid seed number"))
        .collect::<Result<_>>()?;

    let maps = parts
        .map(|block| {
            let (header, conversions) = block.split_once('\n').context("missing map header")?;
            ensure!(header.ends_with(" map:"), "invalid map header");

            conversions
                .lines()
                .map(|line| {
                    let (destination, rest) = line
                        .split_once(|c: char| c.is_ascii_whitespace())
                        .context("missing destination and source separator")?;
                    let (source, range) = rest
                        .split_once(|c: char| c.is_ascii_whitespace())
                        .context("missing source and range separator")?;

                    Ok(Convert {
                        destination: destination.parse().context("invalid destination number")?,
                        source: source.parse().context("invalid source number")?,
                        range: range.parse().context("invalid range number")?,
                    })
                })
                .collect()
        })
...
mod tests {
    ...
    #[test]
    fn part_two() {
        assert_eq!(46, solve_part_two(INPUT).unwrap());
    }
}
