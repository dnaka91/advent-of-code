...
//!   let go of the button. The boat can't move until you let go of the button. Please make sure you
...
pub fn solve_part_one(input: &str) -> Result<u32> {
    Ok(parse_input_one(input)?
        .into_iter()
        .map(|(time, distance)| {
            (1..time)
                .zip((1..time).rev())
                .filter(|(speed, millis)| speed * millis > distance)
                .count() as u32
        })
        .reduce(|a, b| a * b)
        .unwrap_or(0))
...
pub fn solve_part_two(input: &str) -> Result<u64> {
    let (time, distance) = parse_input_two(input)?;

    Ok((1..time).zip((1..time).rev()).filter(|(speed, millis)| speed * millis > distance).count()
        as u64)
}

pub fn parse_input_one(input: &str) -> Result<Vec<(u32, u32)>> {
    let (time, distance) =
        input.trim().split_once('\n').context("missing newline between time and dinstance list")?;

    let time = time
        .strip_prefix("Time:")
        .context("missing time table prefix")?
        .trim()
        .split_ascii_whitespace()
        .map(|v| v.parse().context("invalid time number"));
    let distance = distance
        .strip_prefix("Distance:")
        .context("missing distance table prefix")?
        .trim()
        .split_ascii_whitespace()
        .map(|v| v.parse().context("invalid distance number"));

    time.zip(distance).map(|(time, distance)| Ok((time?, distance?))).collect()
}

pub fn parse_input_two(input: &str) -> Result<(u64, u64)> {
    let (time, distance) =
        input.trim().split_once('\n').context("missing newline between time and dinstance list")?;
    ...
    let distance = distance
        ...
        .collect::<String>()
        .parse()
        .context("invalid distance number")?;
...
mod tests {
    ...
    #[test]
    fn part_one() {
        assert_eq!(288, solve_part_one(INPUT).unwrap());
    }

    #[test]
    fn part_two() {
        assert_eq!(71503, solve_part_two(INPUT).unwrap());
    }
}
