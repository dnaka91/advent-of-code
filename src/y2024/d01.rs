...
//! by a unique number called the **location ID**. To make sure they don't miss anything, The
...
//! right list, the distance apart is `4`; if you pair up a `9` with a `3`, the distance apart is
//! `6`.
//!
...
//! Chief's handwriting, but in the commotion you notice an interesting detail: a lot of location
//! IDs appear in both lists! Maybe the other numbers aren't location IDs at all but rather
...
//! - The fifth number, `3`, appears in the right list three times; the similarity score increases
//!   by **`9`**.
...
pub fn solve_part_one(input: &str) -> Result<u64> {
    let (mut left, mut right) = input
        .lines()
        .map(|line| {
            let (left, right) = line.split_once("   ").context("no separator between pair")?;
            anyhow::Ok((left.parse::<u64>()?, right.parse::<u64>()?))
        })
        .try_fold((Vec::new(), Vec::new()), |(mut acc_left, mut acc_right), pair| {
            let (left, right) = pair?;
            acc_left.push(left);
            acc_right.push(right);
            anyhow::Ok((acc_left, acc_right))
        })?;

    left.sort_unstable();
    right.sort_unstable();

    Ok(left.into_iter().zip(right).map(|(left, right)| left.abs_diff(right)).sum())
}
...
pub fn solve_part_two(input: &str) -> Result<u64> {
    let (left, right) = input
        ...
        .try_fold((Vec::new(), HashMap::default()), |(mut acc_left, mut acc_right), pair| {
            let (left, right) = pair?;
            acc_left.push(left);
            acc_right.entry(right).and_modify(|v| *v += 1).or_insert(1);
            anyhow::Ok((acc_left, acc_right))
        })?;

    Ok(left.into_iter().map(|left| left * right.get(&left).copied().unwrap_or(0)).sum())
}
...
mod tests {
    ...
    #[test]
    fn part_two() {
        assert_eq!(31, solve_part_two(INPUT).unwrap());
    }
}
